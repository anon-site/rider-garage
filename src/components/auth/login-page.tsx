...
export function LoginPage() {
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      const err = login(username.trim(), password, users, remember);
      if (err) setError(err);
      setLoading(false);
    },
    [username, password, login, users, remember]
  );
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Remember me */}
          <div style={{ width: 360, margin: "0 auto 14px", display: "flex", alignItems: "center", gap: 10 }}>
            <button
              type="button"
              onClick={() => setRemember((r) => !r)}
              style={{
                width: 18, height: 18, flexShrink: 0,
                background: remember ? "rgba(0,164,162,0.25)" : "rgba(0,20,20,0.8)",
                border: `1.5px solid ${remember ? "#00fffc" : "#1a4a4a"}`,
                borderRadius: 5,
                display: "flex", alignItems: "center", justifyContent: "center",
                cursor: "pointer", transition: "all 0.25s",
                boxShadow: remember ? "0 0 8px rgba(0,255,252,0.35)" : "none",
              }}
              aria-label="Remember me"
            >
              {remember && (
                <svg width="10" height="8" viewBox="0 0 10 8" fill="none">
                  <path d="M1 4L3.5 6.5L9 1" stroke="#00fffc" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              )}
            </button>
            <span
              onClick={() => setRemember((r) => !r)}
              style={{ color: "#4a8a8a", fontSize: "0.72rem", letterSpacing: "0.12em", cursor: "pointer", userSelect: "none", fontFamily: "Cabin, helvetica, arial, sans-serif" }}
            >
              REMEMBER ME
            </span>
          </div>
