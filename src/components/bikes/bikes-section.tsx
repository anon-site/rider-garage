...
import {
  ...
  UserCheck, X, EyeOff, Warehouse, LayoutGrid, ChevronDown, MapPin, Check,
  Search,
} from "lucide-react";
...
export function BikesSection() {
  ...
  async function handleAddBike(bike: Omit<Bike, "id">) {
    const newId = await addBike(bike);
    if (bike.driverId) {
      await updateDriver(bike.driverId, { bikeId: newId });
    }
  }

  async function handleSaveBike(id: string, changes: Partial<Omit<Bike, "id">>) {
    const prev = bikes.find((b) => b.id === id);
    const oldDriverId = prev?.driverId;
    const newDriverId = changes.driverId;
    await updateBike(id, changes);
    if (oldDriverId && oldDriverId !== newDriverId) {
      await updateDriver(oldDriverId, { bikeId: undefined });
    }
    if (newDriverId && newDriverId !== oldDriverId) {
      await updateDriver(newDriverId, { bikeId: id });
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {!readOnly && showAdd && (
        <AddBikeModal onSubmit={handleAddBike} onClose={() => setShowAdd(false)} />
      )}
