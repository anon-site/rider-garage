...
export function DriversSection() {
  ...
  async function handleAddDriver(driver: Omit<Driver, "id">) {
    const newId = await addDriver(driver);
    if (driver.bikeId) {
      await updateBike(driver.bikeId, { driverId: newId });
    }
  }

  async function handleSaveDriver(id: string, changes: Partial<Omit<Driver, "id">>) {
    const prev = drivers.find((d) => d.id === id);
    const oldBikeId = prev?.bikeId;
    const newBikeId = changes.bikeId;
    await updateDriver(id, changes);
    if (oldBikeId && oldBikeId !== newBikeId) {
      await updateBike(oldBikeId, { driverId: undefined });
    }
    if (newBikeId && newBikeId !== oldBikeId) {
      await updateBike(newBikeId, { driverId: id });
    }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <FilterCard icon={Bike} label="With Bike" value={stats.withBike} tone="emerald" active={filter === "with-bike"} onClick={() => setFilter("with-bike")} />
      ...
      {!readOnly && showAdd && (
        <AddDriverModal onSubmit={handleAddDriver} onClose={() => setShowAdd(false)} />
      )}
